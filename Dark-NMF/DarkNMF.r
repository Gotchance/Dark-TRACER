...
one_packet <- as.numeric(argv[12])              # 1パケットのみのホストの除外有無
PacketLimit_oneSample <-as.numeric(argv[13])     # パケット数の上限(1Sample) 有無
PacketLimit_oneTimeslot<-as.numeric(argv[14])     # パケット数の上限(1Timeslot) 有無

# NMF
basis_rstart <- as.numeric(argv[15])      # 基底数rの始まり
basis_rmax <- as.numeric(argv[16])    # 基底数rの最大値
Tmax <- as.numeric(argv[17])         # 乗法型更新アルゴリズムの反復上限
...
# パラメータ記録
parameter_name <- c("sensor <-", "user_home <-", "startdate <-", "startdate_UNIX <-", "useOctet <-", "MatrixVrow <-", "dataRowTime <-", "ver <-", "one_packet <-", "PacketLimit_oneSample <-", "PacketLimit_oneTimeslot <-", "basis_rstart <-", "basis_rmax <-","Tmax <-")
parameter_value <- c(sensor, user_home, startdate, startdate_UNIX, useOctet, MatrixVrow, dataRowTime, ver, one_packet, PacketLimit_oneSample, PacketLimit_oneTimeslot, basis_rstart, basis_rmax, Tmax)
...
if(useOctet == 4){
    IPv4<-paste(IP[,1],IP[,2],IP[,3],IP[,4],sep=".")
}else if(useOctet == 3){
    IPv4<-paste(IP[,1],IP[,2],IP[,3],sep=".")
}else if(useOctet == 2){
    IPv4<-paste(IP[,1],IP[,2],sep=".")
}

enddate_UNIX<-startdate_UNIX + dataRowTime * (MatrixVrow-1) * 60
colnames(MatrixV)<-IPv4
rownames(MatrixV)<-seq(startdate_UNIX,enddate_UNIX,60*dataRowTime)
# write.table(IP, paste0(result_space,"IPv4_",date,".txt"), quote=F, col.names=F, row.names=F, append=F)
...
write.table(prepro_data[which(is.element(allIPnumeric,IPnumeric)==TRUE),],prepro_file,quote=F,col.names=F,row.names=F,append=F)

try(rm(IPnumeric, allIPnumeric, prepro_data,w, IP, IPv4, enddate_UNIX), silent=TRUE)
...
# # 前処理
# if(one_packet == 1 || PacketLimit_oneSample==1 || PacketLimit_oneTimeslot == 1){
#     if(one_packet == 1){
#         #only more than 2 packets host
#         one_packet_host<-which(colSums(MatrixV)==1)
#         names(one_packet_host) <- NULL
#     }
#     one_packet_hostnum<-length(one_packet_host)
...
#         overPacketLimit_oneTimeslot<-which(apply(MatrixV, 2, sum) > NumofPacketLimit_Timeslot)
#     }
#     PacketLimite_Hosts<-unique(sort(c(one_packet_host,overPacketLimit_oneSample,overPacketLimit_oneTimeslot)))
#     MatrixV<-MatrixV[,-PacketLimite_Hosts]
...
# # # portinfo ALL
# prepro_data <- read.table(prepro_file)
# # unique_dstPort_countpacket<-NULL
# # unique_dstPort_countsrcIP<-NULL
# # unique_dstPort <- sort(unique(prepro_data[,8]))

# # IPの数値化 (1.1.1.1 -> 001001001001), srcIPdstPortのunique, srcIPのunique
# if(useOctet == 4){
#     allIPnumeric<-c(as.numeric(paste0(formatC(prepro_data[,4], width=3, flag="0"),formatC(prepro_data[,5], width=3, flag="0"),formatC(prepro_data[,6], width=3, flag="0"),formatC(prepro_data[,7], width=3, flag="0"))))
#     unique_srcIPdstPort <- unique(prepro_data[,4:8])
#     unique_srcIP <- unique(prepro_data[,4:7])
# }else if(useOctet == 3){
#     allIPnumeric<-c(as.numeric(paste0(formatC(prepro_data[,4], width=3, flag="0"),formatC(prepro_data[,5], width=3, flag="0"),formatC(prepro_data[,6], width=3, flag="0"))))
#     unique_srcIPdstPort <- unique(prepro_data[,c(4,5,6,8)])
#     unique_srcIP <- unique(prepro_data[,4:6])
# }else if(useOctet == 2){
#     allIPnumeric<-c(as.numeric(paste0(formatC(prepro_data[,4], width=3, flag="0"),formatC(prepro_data[,5], width=3, flag="0"))))
#     unique_srcIPdstPort <- unique(prepro_data[,c(4,5,8)])
#     unique_srcIP <- unique(prepro_data[,4:5])
# }

# # # unique dstPort別のパケット数，srcIP数カウント
# # for(k in 1:length(unique_dstPort)){
# #     unique_dstPort_countpacket[k] <- length(which(prepro_data[,8]==unique_dstPort[k]))
# #     if(useOctet == 4){
# #         unique_dstPort_countsrcIP[k] <- length(which(unique_srcIPdstPort[,5]==unique_dstPort[k]))
# #     }else if(useOctet == 3){
# #         unique_dstPort_countsrcIP[k] <- length(which(unique_srcIPdstPort[,4]==unique_dstPort[k]))
# #     }else if(useOctet == 2){
# #         unique_dstPort_countsrcIP[k] <- length(which(unique_srcIPdstPort[,3]==unique_dstPort[k]))
# #     }
# # }

# # # unique dstPort別のパケット数，srcIP数パーセンテージ
# # unique_dstPort_percentpacket <- round(unique_dstPort_countpacket/sum(unique_dstPort_countpacket) * 100, digits = 2)
# # unique_dstPort_percentsrcIP <- round(unique_dstPort_countsrcIP/nrow(unique_srcIP) * 100, digits = 2)

# # packet_by_unique_dstPort <- cbind(unique_dstPort, unique_dstPort_countpacket, unique_dstPort_percentpacket)
# # packet_by_unique_dstPort <- packet_by_unique_dstPort[order(-packet_by_unique_dstPort[,2]),]

# # srcIP_by_unique_dstPort <- cbind(unique_dstPort, unique_dstPort_countsrcIP, unique_dstPort_percentsrcIP)
# # srcIP_by_unique_dstPort <- srcIP_by_unique_dstPort[order(-srcIP_by_unique_dstPort[,2]),]

# # write.csv(packet_by_unique_dstPort, paste0(result_space,"PortinfoALL_packet.csv"), row.names=F)
# # write.csv(srcIP_by_unique_dstPort, paste0(result_space,"PortinfoALL_srcIP.csv"), row.names=F)
# # #################
